...
const SECTIONS = [
  ...
  },
  {
    ...
    items: [
      ...
      ['Interior sketch', 'Placed envelopes show their rooms as shaded cells (a Voronoi of the Concept layout — the 👁 dock button toggles it). Each cell reads its area against the room’s target and turns red-dashed when the room doesn’t fit there. Click a cell to select its room (the Link tool works on cells too); dragging a cell moves the whole building.'],
      ['Re-plan a room', 'Drag a cell’s dot to move the room inside its envelope — the move saves back to the Concept view and pins the room there.'],
      ['One storey at a time', 'With levels assigned, the Interior selector in the toolbar shows a single storey’s rooms inside each envelope — the honest read, since the footprint is one floor plate. “All floors” overlays every storey.'],
